...
import slides.layout.types.ColumnsType;
import slides.layout.types.GridType;
import slides.layout.types.NormalType;
import slides.model.*;
...
public class Menu {
	...
	private MasterManager manager = new MasterManager();
	...
	public Menu(Presentation presentation) {
		this(presentation, new InputStreamReader(System.in), new OutputStreamWriter(System.out));
		canvas = new Canvas();
		interactive = true;
	}
	...
	private void showSlideNumber() {
		canvas.drawLine(String.format("Diapositiva %d de %d", presentation.currentSlideNumber(),
	...
	private void newMasterLayout() {
		...
		presentation.addSlide(slide);
		showCurrentSlide();
		System.out.println("Introduzca el nombre de la diapositiva: ");
		String name = askUser();
		manager.createMaster(name, slide);
	...
	private void newSlideFromMaster() {
		System.out.println("Introduzca el nombre de la diapositiva: ");
		String name = askUser();
		Slide master = manager.getMaster(name);
		presentation.addSlide(master);
		showCurrentSlide();
		// TODO: Completarlo:
	 ...
	 * errores de entrada/salida (que no se deberían producir si se está leyendo de
	 * la entrada estándar).
	 */
	 ...
	 * debería ser quien hubiera llamado el método 'run' de esta clase. En caso de
	 * que se esté escribiendo la salida del programa en un fichero en vez de en la
	 * consola, lo cierra.
	 */
	private void exit() {
		exit = true;
