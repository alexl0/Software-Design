...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		// Cambios: Ninguno
		// -------------------------------------------------------------------
		system.copyFile("prueba1.txt",
				new LinesFilter(new Fork(new FileOutput("a.txt"), new Encryptor(new FileOutput("b.txt")))));

		// -- Opcional 2
		// Cambios: Ninguno (nos podemos arreglar con un fork de forks)
		// -------------------------------------------------------------------
		system.copyFile("privado.txt", new Fork(new LinesFilter(new FileOutput("privado2.txt")), new Encryptor(
				new Fork(new Internet("156.17.11.196"), new SpacesFilter(new Bluetooth("Galaxy de Raúl"))))));
