	...
	public void readConfigurationFile() throws LoaderException {
		try {
			...
			while ((line = configFile.readLine()) != null) {
				...
				registerInstructionFactory(instructionName,
						(InstructionFactory) instructionFactoryClass.getDeclaredConstructor().newInstance());
			}
			configFile.close();
		} catch (FileNotFoundException e) {
			throw new LoaderException(
					"No se encontró el fichero de configuración: no se pueden cargar las instrucciones");
		} catch (IOException e) {
			throw new LoaderException(
					"Error leyendo el fichero de configuración: no se pueden cargar las instrucciones");
		} catch (ClassNotFoundException e) {
			throw new LoaderException(
					"Error al cargar las instrucciones: no se encontró la factoría especificada: " + e.getMessage());
		} catch (Exception e) {
			throw new LoaderException(
					"Error al cargar las instrucciones: no se pudo crear la factoría: " + e.getMessage());
		}
	}
